...
import publishToUnlikeSubscribers from "../redis/pubsub/pub/unlike.publisher";
...
async function unlikeService(payload: any, decodedPayload: IDecodedPayload) {
  ...
  const unlikePayload = Object.preventExtensions({
    unLikeBy: userId,
    postId: postId,
  });

  const publishStatus = await publishToUnlikeSubscribers(
    unlikePayload,
    redisClient
  );
  ...
  if (isSubscriberFailed) {
    const errorMessage = `The Message cannot be publish to the Un Like Subscriber`;
    graphLogger.info(errorMessage);
  }
  return {
    post: includeKeyIntoObjects(postDocument._doc, "title", "description"),
